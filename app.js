...
const mongoUrl = "mongodb+srv://tegarraihanakmali:admintegar@cluster0.1jzdsvb.mongodb.net/?retryWrites=true&w=majority"
...
app.post("/createschedule", async (req, res) => {
      ...
      try {
            const newSchedule = await mongoose.model("Laporan").create({
                  tgl,
                  hari,
                  matkul,
                  jam,
...
app.get("/getschedules", async (req, res) => {
      try {
            const schedules = await mongoose.model("Laporan").find();
...
app.post("/updateschedule", async (req, res) => {
      ...
      try {
            const updatedSchedule = await mongoose.model("Laporan").findOneAndUpdate(
                  { _id: scheduleId },
...
app.delete("/deleteschedule/:scheduleId", async (req, res) => {
      ...
      try {
            const deletedSchedule = await mongoose.model("Laporan").findByIdAndDelete(scheduleId);
